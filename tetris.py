...
class Shape():
    ...
    def can_rotate(self, board):
        ...
        for block in self.blocks:
            x = self.center_block.x - dir*self.center_block.y + dir*block.y
            y = self.center_block.y + dir*self.center_block.x - dir*block.x
...
class Board():
    ...
    def game_over(self):
        ...
        '''
        #YOUR CODE HERE
        P = Point(self.width/2*(Block.BLOCK_SIZE),self.height/2*Block.BLOCK_SIZE)
...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        '''
        #YOUR CODE HERE
        ns = random.choice(self.SHAPES)
        p = Point(int(self.BOARD_WIDTH/2),0)
        new_shape = ns(p)
        return new_shape
    ...
    def key_pressed(self, event):
        ...
        #YOUR CODE HERE
        key = event.keysym
        #keys are down, left, right
        if key in self.DIRECTION:
            self.do_move(key)
        #keys is space
        elif key == "space":
            while self.current_shape.can_move(self.board, Tetris.DIRECTION['Down'][0], Tetris.DIRECTION['Down'][1]):
                self.do_move("Down")
            self.do_move("Down")
        #key is up
        else:
